#!/usr/bin/env python3
"""Train the M3GNet XAS FEFF showcase pipeline on the v2 Figshare package (scratch encoder).

This is a versioned copy of ``train_m3gnet_xas_pipeline.py`` adapted to the v2
Figshare target package written by ``extract_figshare_feff_v2.py``. Differences
from the showcase pipeline:

- ``--data-root`` points at the v2 package directory, which contains the
  showcase-style ``tutorial_omnixas/ml_data`` and
  ``tutorial_omnixas/material_id_and_site`` subdirectories plus
  ``extracted/FEFF/<element>/<material>/POSCAR`` structures.
- The preflight does not require canonical X feature files (the encoder is
  trained from scratch and exports its own features).
- The encoder always starts from random initialization; no pretrained weights
  are ever loaded. This is recorded in ``provenance.json``.

Everything else follows the showcase pipeline: balanced scratch ``M3GNetXAS``
encoder training, 64D feature export, balanced UniversalXAS training, and
validation-selected tuned heads.
"""
...
DEFAULT_EPOCHS = 1000
PACKAGE_MARKER = "PACKAGE_COMPLETE.json"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--data-root", required=True, help="v2 package directory written by extract_figshare_feff_v2.py")
    p.add_argument("--run-name", default=None)
    p.add_argument("--output-root", default="output/training/m3gnet_xas_pipeline_v2")
...
def data_root_path(args: argparse.Namespace) -> Path:
    root = Path(args.data_root)
    root = root if root.is_absolute() else Path.cwd() / root
    if not root.is_dir():
        raise FileNotFoundError(f"Data root (v2 package) not found: {root}")
    if not (root / PACKAGE_MARKER).is_file():
        raise FileNotFoundError(f"Missing {PACKAGE_MARKER}; the package is incomplete: {root}")
    return root


def preflight(data_root: Path, raw: Path, require_raw: bool = True) -> None:
    data = data_root / "tutorial_omnixas/ml_data"
    ids_dir = data_root / "tutorial_omnixas/material_id_and_site"
    counts: dict[str, int] = {}
    missing: list[str] = []
    ...
    if missing:
        raise FileNotFoundError("Missing v2 package data files:\n" + "\n".join(missing[:12]))
    ...
    print(json.dumps({
        ...
        "train_rows_per_element": counts,
        "feature_dim": FEATURE_DIM,
        "spectrum_dim": SPECTRUM_DIM,
        "encoder_parameters": params,
        "head_parameters": head_params,
        "encoder_init": "scratch (random init); no pretrained weights loaded",
        "raw_root": str(raw),
        "raw_root_exists": raw.is_dir(),
    }, indent=2))
    if require_raw:
        if not raw.is_dir():
            raise FileNotFoundError(f"Missing raw FEFF structure root: {raw}. Point --data-root at the v2 package.")
        validate_raw_structures(data_root, raw, FEFF_TASKS)
    print("preflight passed: no training was started")


class LitScratch(pl.LightningModule):
    """Balanced, baseline-normalized scratch encoder training (showcase recipe)."""
...
def baselines(data_root: Path) -> tuple[torch.Tensor, torch.Tensor]:
    d = data_root / "tutorial_omnixas/ml_data"
    train, val = [], []
    for task in FEFF_TASKS:
...
def validate_features(features: Path, data_root: Path) -> None:
    canonical = data_root / "tutorial_omnixas/ml_data"
    for task in FEFF_TASKS:
        ...
        for name in SPLITS:
            ...
            if not np.array_equal(part.y, expected_y):
                raise ValueError(f"Exported targets do not exactly match package targets: {task} {name}")
...
def main() -> None:
    ...
    data_root = data_root_path(args)
    output = Path(args.output_root); output = output if output.is_absolute() else Path.cwd() / output
    if (args.resume or args.evaluate) and args.run_name is None:
        raise ValueError("--resume and --evaluate require --run-name")
    run = output / (args.run_name or f"m3gnet_xas_v2_seed{args.seed}")
    if args.evaluate:
        ...
        validate_features(features, data_root)
    ...
    if args.precompute_graphs:
        ...
        graph_cache = pgc.default_out_dir(data_root, probe.cutoff, probe.threebody_cutoff)
        print(f"--precompute-graphs: encoder cutoff={probe.cutoff} threebody_cutoff={probe.threebody_cutoff}")
        pgc.ensure_graph_cache(data_root, raw, graph_cache, probe.cutoff, probe.threebody_cutoff,
    ...
    patch_matgl_gpu_constants(); preflight(data_root, raw, require_raw=graph_cache is None)
    ...
    if not args.resume:
        package_report = data_root / "report.json"
        (run / "provenance.json").write_text(json.dumps({
            "pipeline": "m3gnet_xas_pipeline_v2",
            "encoder_init": "scratch (random init); no pretrained weights loaded",
            "data_root": str(data_root),
            "package_report": json.loads(package_report.read_text(encoding="utf-8")) if package_report.is_file() else None,
            "tasks": FEFF_TASKS,
    ...
    train_base, val_base = baselines(data_root)
    ...
    def make_graph_dataset(tasks, split, encoder):
        if graph_cache is not None:
            return CachedGraphDataset(data_root, graph_cache, tasks, split, encoder.cutoff, encoder.threebody_cutoff, encoder.element_types)
        return FEFFDataset(data_root, raw, tasks, split)
